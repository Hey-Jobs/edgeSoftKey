...
export async function getAdminCardsPaged(params: {
  ...
  return {
    ...
    items: cards.map((item) => ({
      ...
      createdAt: item.createdAt.toISOString(),
      contentPreview: previewCard(item.content),
      validity: item.cardValidity.length > 0
        ? {
            expireAt: item.cardValidity[0].expireAt.toISOString(),
            boundAt: item.cardValidity[0].boundAt.toISOString(),
          }
        : null,
      binding: item.hardwareBinding.length > 0 ? {
        id: item.hardwareBinding[0].id,
        hardwareHash: item.hardwareBinding[0].hardwareHash,
        status: item.hardwareBinding[0].status,
        lastVerifiedAt: item.hardwareBinding[0].lastVerifiedAt
          ? item.hardwareBinding[0].lastVerifiedAt.toISOString()
          : null,
        createdAt: item.hardwareBinding[0].createdAt.toISOString(),
      } : null,
    })),
  };
}
